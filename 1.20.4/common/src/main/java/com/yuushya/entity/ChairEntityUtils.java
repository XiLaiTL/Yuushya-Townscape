...
public class ChairEntityUtils {
    ...
    public static boolean isValidityLocation(LevelAccessor world, Vec3 pos, Pose passengersPose ) {
        return isValidityLocation( world, new BlockPos((int) Math.floor( pos.x( ) ), (int) Math.floor( pos.y( ) - 0.03 ), (int) Math.floor( pos.z( ) )), passengersPose ) ;
    } ;
    /** 坐标合理性判断
     *
     * 同上，只不过pos换成了检测点
    ...
    public static boolean isValidityLocation(LevelAccessor world, BlockPos pos, Pose passengersPose ) {
        return !(world.getBlockState( pos ).getBlock() instanceof AirBlock) && ( passengersPose.equals( Pose.STANDING ) || passengersPose.equals( Pose.CROUCHING ) ) ;
    } ;
    /** 坐标合理性判断
     *
    ...
    public static boolean isValidityLocation( LivingEntity entity ) {
        return entity.onGround( ) && isValidityLocation( entity.level(), entity.position( ), entity.getPose( ) ) ;
    } ;
