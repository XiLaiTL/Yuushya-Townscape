...
public class GetMixedBlockEntityItem extends AbstractYuushyaItem {
    ...
    public GetMixedBlockEntityItem(Settings settings, int linecount) {
...
//        Item item = offhand.getItem();
        ...
        if(block instanceof MixedBlock||block instanceof ShowBlock) {
            if (player != null) {
                player.sendMessage(new TranslatableText("item.yuushya.get_mixedblock_item.pass"), true);
            }
            ...
            if (player != null) {
                player.sendMessage(new TranslatableText("item.yuushya.get_mixedblock_item.success"), true);
            }
