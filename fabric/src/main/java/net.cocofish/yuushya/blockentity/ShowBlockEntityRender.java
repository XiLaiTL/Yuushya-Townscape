...
public class ShowBlockEntityRender implements BlockEntityRenderer<ShowBlockEntity> {
    ...
    public ShowBlockEntityRender(BlockEntityRendererFactory.Context ctx) {}
    ...
    public void render(ShowBlockEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        blockRenderManager.getModelRenderer().render(world, blockRenderManager.getModel(blockState), blockState, blockPos, matrices, vertexConsumers.getBuffer(RenderLayers.getMovingBlockLayer(blockState)), false, this.random, blockState.getRenderingSeed(blockEntity.getPos()), OverlayTexture.DEFAULT_UV);
