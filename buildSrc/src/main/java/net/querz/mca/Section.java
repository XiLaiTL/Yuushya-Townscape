...
public class Section extends SectionBase<Section> {
	...
	public Section(CompoundTag sectionRoot, int dataVersion, long loadFlags) {
		...
		if(dataVersion > DataVersion.JAVA_1_18_21W43A.id()){
			...
			if ((loadFlags & BLOCK_STATES) != 0) {
				CompoundTag block_states = sectionRoot.getCompoundTag("block_states");
				if (block_states != null){
					LongArrayTag blockStates = block_states.getLongArrayTag("data");
					if (blockStates != null) this.blockStates = blockStates.getValue();
					ListTag<?> rawPalette = block_states.getListTag("palette");
					if (rawPalette == null) {
						return;
					}
	...
	public CompoundTag updateHandle(int y) {
		...
		if(dataVersion > DataVersion.JAVA_1_18_21W43A.id()){
			...
			CompoundTag block_states = data.getCompoundTag("block_states");
			if(block_states!=null){
				if (palette != null) {
